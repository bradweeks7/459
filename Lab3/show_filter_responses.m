...
fprintf('\nMinimum Magnitude = %d', abs(lin_min));
...
fprintf("\nMagnitude @ 3dB Cutoff Frequency = %d", mag_3dB); 

% Find filter type
% Based on Peak and Min Magnitude Freqs, finds and prints 3dB freq + BW
if freq_min > 0.1 && freq_min < 0.4
    fprintf("\nFilter Typer: Band-Stop (Notch)\n\n");
    ...
    fprintf("\n3 dB Frequencies (cycles/sample) = %d", freq_3dB);
    fprintf("\n3 dB Bandwidth (cycles/sample) = %d", bw);

elseif freq_max > 0.1 && freq_min < 0.5
    fprintf("\nFilter Typer: Band-Pass\n\n");
    indices_3dB = find(HF_dB <= maxval-3);
    freq_under_3dB = Fd(indices_3dB);
    freq_3dB = [freq_under_3dB(1), freq_under_3dB(length(freq_under_3dB))];
    bw = freq_3dB(2) - freq_3dB(1);
